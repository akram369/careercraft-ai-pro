...
def _extract_text_from_obj(obj):
    ...
    try:
        ...
        if hasattr(obj, "candidates"):
            ...
            if c and len(c)>0:
                cand = c[0]
                if hasattr(cand, "content"): return cand.content
                if isinstance(cand, dict) and "content" in cand: return cand["content"]
    except Exception: pass
    try:
...
def _try_openai(prompt, model="gpt-4o-mini", max_retries=3):
...
def _try_gemini(prompt, candidate_models=None):
    ...
    if candidate_models is None:
        candidate_models = ["gemini-1.5","gemini-1.5-flash","gemini-1.0","chat-bison@001","text-bison@001"]
    try:
        if hasattr(genai, "list_models"):
            avail = genai.list_models()
            names = []
            for m in avail:
                try: names.append(m.name if hasattr(m,"name") else (m.get("name") if isinstance(m,dict) else None))
                except Exception: continue
            found = [n for n in candidate_models if n in (names or [])]
            if found: candidate_models = found + [m for m in candidate_models if m not in found]
    except Exception:
        pass
    last_err = None
...
def get_ai_response(prompt, model="gpt-4o-mini"):
    ...
    if not guidance: guidance.append("Check provider availability, keys, and client library versions.")
